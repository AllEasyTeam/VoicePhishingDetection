...
def evaluate(model, df, feature_cols=None, threshold_df=None, pr_auc_method="trapezoidal", calibrator=None):
    ...
    # 1. 보정기가 있으면 보정된 확률을, 없으면 원본 확률을 추출
    if calibrator is not None:
        proba_thr = calibrator.predict_proba(X_thr)[:, 1]
    else:
        proba_thr = model.predict_proba(X_thr)[:, 1]

    # 2. [유지됨] 질문하신 핵심 로직은 단 한 글자도 빠짐없이 그대로 들어 있습니다.
    precisions, recalls, thresholds = precision_recall_curve(y_thr, proba_thr)
    if len(thresholds) == 0:
        best_threshold = 0.5
    else:
...
# Simulator/Detection/train_eval_cali.py 하단 fit_calibrator 함수 수정

def fit_calibrator(model, val_df, feature_cols, target_col="is_phishing", method="isotonic"):
    """
    scale_pos_weight로 인해 왜곡된 점수를 실제 자연 확률(0~1)로 정렬하는 보정기를 적합.
    최신 Scikit-Learn의 cv='prefit' 제거 이슈를 완벽히 호환 처리함.
    """
    feature_cols = _resolve_feature_cols(feature_cols)
    ...
    try:
        # 최신 Scikit-Learn 방식: FrozenEstimator로 모델을 동결하고 cv=None 적용
        ...
        # Scikit-Learn 1.2~1.4 과도기 버전 및 레거시 대응
        # FrozenEstimator가 없거나 지원되지 않는 경우
        calibrated_model = CalibratedClassifierCV(
            estimator=model,
            method=method,
            cv=None
