...
from Simulator.Generation.generator_utils import get_soph, phone_from_category
...
def _generate_phishing_phone(
    ...
    soph = get_soph(sophistication, config.SOPH_NUMBER_TYPE_BAND)
    ...
    if first_contact_type == 0 and isinstance(sms_band, dict):  # 0: 문자(sms)
        ...
        kind = _pick_from_call_band(config, soph)
    return phone_from_category(kind)
...
def generate_phishing_event(p_type: str, sophistication: Union[str, Dict[str, str]], config) -> dict:
    ...
    num_soph = get_soph(sophistication, config.SOPH_NUM_IN_MSG)
    ...
    sms_soph = get_soph(sophistication, config.SOPH_SMS_TO_CALL)
    sms_to_call = 1 if (
