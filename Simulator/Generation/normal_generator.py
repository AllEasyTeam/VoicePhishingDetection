...
def generate_normal_event(subgroup: str, config, sophistication: Union[str, Dict[str, str]] = "중간") -> dict:
    ...
    call_type = random.choice([0, 1])
    first_contact_type = random.choice(["call", "sms"])
    is_global = 0

    # 3. 연락처 저장 및 과거 통화 이력 (config.F_GROUP)
    in_contacts = 1 if random.random() < config.F_GROUP["연락처_저장확률"][subgroup] else 0
    has_prior_history = 1 if random.random() < config.F_GROUP["과거통화이력_있음확률"][subgroup] else 0

    # 4. 재연락 간격 (규칙 1: 과거 통화 이력 없으면 NaN)
    if has_prior_history == 1:
        repeat_gap = round(random.uniform(1.0, 1440.0), 2)
    else:
        repeat_gap = np.nan
    ...
    sms_to_call = 1 if (first_contact_type == "sms" and random.random() < 0.25) else 0
    if sms_to_call == 1:
